...
\subtitle{Lecture 1: Counterfactual Framework and Selection on Observables\\
...
\begin{frame}{Hume's challenge: we infer causation, we never observe it}
      \small Hume, \emph{Treatise} (1739) \& \emph{Enquiry} (1748):
      \begin{itemize}
        \item We observe \textbf{sequences}: flame touches paper $\to$ paper burns
        \item We observe \textbf{regularities}: this always happens
        \item We \emph{never} observe the necessary connection itself
        \item ``Causal power'' is \alert{constant conjunction + habit of expectation} ---
              a feature of our \emph{minds}, not the world
      \end{itemize}
      \smallskip
      \begin{block}{\small Hume's inescapable lesson}
        \footnotesize
        Every causal claim adds assumptions \textbf{beyond what the data contain.} The task: make those assumptions explicit.
      \end{block}
\end{frame}

% ---- 300-year intellectual roadmap ---------------------------

\begin{frame}{Three centuries of causal thinking: a roadmap}

  {\footnotesize\renewcommand{\arraystretch}{1.35}%
  \begin{tabular}{@{}p{2.9cm} p{1.6cm} p{8.4cm}@{}}
    \toprule
    \textbf{Who} & \textbf{When} & \textbf{Key move} \\
    \midrule
    Hume              & 1739          & Stated the problem: causation is inferred, never directly observed \\
    Mill              & 1843          & Method of difference: compare units differing in one cause only \\
    Galton \& Pearson & 1880s--1900s  & Formalised correlation --- powerful tool, but \alert{conflated} with causation \\
    Fisher            & 1925          & Randomisation: \emph{design} beats analysis --- balance confounders by chance \\
    Neyman / Rubin    & 1923\,/\,1974 & Potential outcomes $Y_i(1)-Y_i(0)$: put the counterfactual at the centre \\
    Haavelmo / Cowles & 1944--55      & Structural models + IV: theory-driven identification from observational data \\
    Pearl             & 1995          & DAGs: graphical language for assumptions and identification conditions \\
    Angrist, Card, Krueger & 1990s--  & \textbf{Credibility revolution}: quasi-experiments, design-based identification \\
    \bottomrule
  \end{tabular}}
\end{frame}
...
\begin{frame}{The two-way failure of correlation}
  \centering
  \includegraphics[width=\textwidth,height=0.80\textheight,keepaspectratio]{../Figures/L1_fallacy.png} \\
  \smallskip
  {\scriptsize Source: \citet{chavez2026}}
\end{frame}
...
\begin{frame}{Three roads to a causal answer}
  ...
  \begin{block}{Lecture 1 (Today): Selection on observables}
...
% --- Casual methods  over time -----------------------------------------
\begin{frame}[shrink=5]{Casual methods over time}
  \centering
  \includegraphics[width=0.7\textwidth]{../Figures/L1_did_growth1.png} \\
...
\begin{frame}{Rubin's potential outcomes notation}
  ...
  \bigskip
  \textbf{Switching equation} — what we actually observe (that is $Y_i$):
  \[
...
\begin{frame}{Potential outcomes: a complete (fictional) table}

  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{tabular}{lcccc}
        \toprule
        Unit & $D_i$ & $Y_i(1)$ & $Y_i(0)$ & $\tau_i$ \\
        \midrule
        Alice  & 1 & 1 & 0 & \phantom{$-$}1 \\
        Bob    & 1 & 0 & 0 & \phantom{$-$}0 \\
        Carol  & 0 & 1 & 1 & \phantom{$-$}0 \\
        Dan    & 0 & 0 & 0 & \phantom{$-$}0 \\
        \bottomrule
      \end{tabular}

      \vspace{3mm}
      \small In a fictional world we \emph{observe both} potential outcomes for every unit.
    \end{column}
    \begin{column}{0.44\textwidth}
      \begin{exampleblock}{Exercise}
        Using the complete table, compute:
        \begin{itemize}
          \item $\text{ATE} = ?$
          \item $\text{ATT} = ?$ \hfill (treated: Alice, Bob)
          \item $\text{ATU} = ?$ \hfill (untreated: Carol, Dan)
        \end{itemize}
        \vspace{2mm}
        \small Are they equal? What does that tell you?
      \end{exampleblock}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Potential outcomes: exercise solution}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{tabular}{lcccc}
        ...
        Dan    & 0 & 0 & 0 & 0 \\
        \midrule
        \multicolumn{4}{l}{\small $\text{ATE}$} & $\frac{1+0+0+0}{4} = \mathbf{0.25}$ \\[4pt]
        \multicolumn{4}{l}{\small $\text{ATT}$ (Alice, Bob)} & $\frac{1+0}{2} = \mathbf{0.50}$ \\[4pt]
        \multicolumn{4}{l}{\small $\text{ATU}$ (Carol, Dan)} & $\frac{0+0}{2} = \mathbf{0.00}$ \\
        \bottomrule
      \end{tabular}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{Are they equal?}
        No: $\text{ATT} > \text{ATE} > \text{ATU}$.
      \end{alertblock}

      \vspace{3mm}
      \textbf{What does that tell you?}\\[3pt]
      Units who \emph{chose} treatment (Alice, Bob) gain more from it than those who did not. This is \textbf{positive selection into treatment}.

      \vspace{3mm}
      In the real world we cannot compute any of these directly — we observe only \emph{one} potential outcome per unit. That is why identification strategy matters.
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \begin{block}{Naive solution}
        Simple difference in outcomes: compares treated to untreated directly
      \end{block}
    \end{column}
...
\begin{frame}[shrink=10]{Selection bias: what goes wrong with naive comparisons}
  ...
  \onslide<4->
    ...
    \smallskip
    \begin{itemize}
      \item where $\pi$ is the share of treated units
...
\begin{frame}[shrink=3]{A first attempt: just add controls to OLS}
  ...
  \onslide<1>
    \begin{columns}[T]
      ...
      \begin{column}{0.36\textwidth}
        ...
        % DAG: X is a positive confounder — backdoor paths in red dashed
        \begin{tikzpicture}[
            dn/.style={circle, draw=ecblue, fill=lightblue, thick,
                       minimum size=8mm, font=\small\bfseries},
            da/.style={-{Stealth[length=5pt]}, thick, ecnavy},
            bk/.style={-{Stealth[length=5pt]}, thick, ecred, dashed}]
          \node[dn] (X) at (1.0, 1.8) {$X$};
          \node[dn] (D) at (0.0, 0.0) {$D$};
          \node[dn] (Y) at (2.0, 0.0) {$Y$};
          \draw[bk] (X) -- (D) node[midway, left,  font=\footnotesize] {$+$};
          \draw[bk] (X) -- (Y) node[midway, right, font=\footnotesize] {$+$};
          \draw[da] (D) -- (Y) node[midway, below, font=\footnotesize] {$\beta$};
        \end{tikzpicture}\\[3pt]
        {\scriptsize\color{ecred} $X$ biases $\hat\beta^{\text{naive}}$ upward}
      \end{column}
    \end{columns}
  ...
  \onslide<2>
    \begin{columns}[T]
      ...
      \begin{column}{0.36\textwidth}
        \centering\vspace{4pt}
        % DAG: X→D positive, X→Y negative — backdoor attenuates true effect
        \begin{tikzpicture}[
            dn/.style={circle, draw=ecblue, fill=lightblue, thick,
                       minimum size=8mm, font=\small\bfseries},
            da/.style={-{Stealth[length=5pt]}, thick, ecnavy},
          ...
          \draw[da] (D) -- (Y) node[midway, below, font=\footnotesize] {$\beta$};
        \end{tikzpicture}\\[3pt]
        {\scriptsize\color{ecred} $X$ biases $\hat\beta^{\text{naive}}$ downward}
      \end{column}
    \end{columns}
  ...
  \onslide<3>
    \begin{columns}[T]
      \begin{column}{0.60\textwidth}
        \begin{alertblock}{Sign flip — the most alarming case}
          $\hat\beta^{\text{naive}} > 0$ but $\hat\beta^{\text{OLS+X}} < 0$ (or vice versa).
          Omitted variable so strongly correlated with $D$ that it \emph{reversed} the raw association.\\[2pt]
      ...
      \begin{column}{0.36\textwidth}
        \centering\vspace{4pt}
        % DAG: backdoor dominates — strong confounding reverses sign
        \begin{tikzpicture}[
            dn/.style={circle, draw=ecblue, fill=lightblue, thick,
                       minimum size=8mm, font=\small\bfseries},
            da/.style={-{Stealth[length=5pt]}, thick, ecnavy},
          ...
          \node[dn] (D) at (0.0, 0.0) {$D$};
          \node[dn] (Y) at (2.0, 0.0) {$Y$};
          \draw[bk] (X) -- (D) node[midway, left,  font=\footnotesize] {$\boldsymbol{++}$};
          \draw[bk] (X) -- (Y) node[midway, right, font=\footnotesize] {$\boldsymbol{{-}{-}}$};
          \draw[da] (D) -- (Y) node[midway, below, font=\footnotesize] {$\beta$};
        \end{tikzpicture}\\[3pt]
        {\scriptsize\color{ecred} backdoor overwhelms $\beta$ $\Rightarrow$ sign flip}
      \end{column}
    \end{columns}
...
\begin{frame}{Inverse Probability Weighting (IPW)}
  ...
  This a weighted OLS, with weights
...
% ------------------------------------------------------------
\begin{frame}[shrink=8]{Dataset 1: the Titanic}

  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \textbf{Background}\\[3pt]
      RMS Titanic sank on 15 April 1912. Of the 2{,}224 people on board, 1{,}502 died. Evacuation followed a ``women and children first'' protocol; lifeboat access differed sharply by deck.

      \smallskip
      \textbf{Data} \citep{Dawson1995}\\[3pt]
      Passenger manifest: ticket class (1st/2nd/3rd), sex, age group, survival indicator.

      \smallskip
      \textbf{Causal question}\\[3pt]
      Did being a \emph{first-class} passenger \emph{cause} higher survival --- or does the raw gap reflect compositional differences in sex and age across classes?
    \end{column}
    \begin{column}{0.42\textwidth}
      \begin{block}{Key results (raw data)}
        \begin{tabular}{lc}
          \toprule
          Group & Survival rate \\
          \midrule
      ...
      \begin{alertblock}{The identification problem}
        1st-class passengers were disproportionately \emph{female} and \emph{adult} — both independently associated with survival. Naive gap conflates class effect with composition.
      \end{alertblock}
    \end{column}
  \end{columns}
...
% ------------------------------------------------------------
\begin{frame}[shrink=8]{Dataset 2: LaLonde (1986) and the NSW programme}

  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \textbf{Background}\\[3pt]
      NSW demonstration (mid-1970s): \emph{randomised} job-training for disadvantaged workers — long-term unemployed, ex-offenders, school drop-outs. Random assignment into training ($D=1$) or control ($D=0$).

      \smallskip
      \textbf{Data} \citep{lalonde1986}\\[3pt]
      \begin{itemize}
        \item \textbf{Experimental:} NSW treated + NSW controls
        \item \textbf{Observational:} NSW treated + CPS/PSID controls
        \item Covariates: age, education, race, marital status, earnings 1974--75
        \item Outcome: real earnings 1978
      \end{itemize}

      \smallskip
      \textbf{Experimental benchmark}\\[3pt]
      Training raised earnings by $\approx\$1{,}794$ (statistically significant).
    \end{column}
    \begin{column}{0.42\textwidth}
      \begin{alertblock}{LaLonde's bombshell \citep{lalonde1986}}
        Swapping to CPS/PSID controls and re-running OLS and Heckman selection: estimates ranged from $-\$15{,}000$ to $+\$3{,}500$.\\[3pt]
        Observational methods fail when selection is driven by unobservables.
      \end{alertblock}
      \smallskip
      \begin{block}{The rehabilitation \citep{dehejia2002}}
        PS matching on the same observational data recovered estimates close to \$1{,}794 — launching the matching literature and the debate on when CIA is credible.
      \end{block}
    \end{column}
  \end{columns}
